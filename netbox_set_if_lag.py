...
def main():
  ...
  parser.add_argument('-L', '--lag-iface', help='Parent LAG interface, eg. "Po 3"', required=True)
  ...
  # find lag interface
  lags = nb.dcim.interfaces.filter(device=args.host, name=args.lag_iface)
  if len(lags) == 0:
    fail("LAG interface does not exist")
  if len(lags) > 1:
    fail("too many LAG interfaces found")

  # find vdc
  lag = next(lags)
  for iface in ifaces:
    if iface.lag != lag:
      print("set lag for", iface, "->", lag)
      iface.lag = lag
      iface.save()
